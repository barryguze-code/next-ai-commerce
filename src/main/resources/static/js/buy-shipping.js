(()=>{
  function orderShortcutStyle(){
    ...
    const style=document.createElement('style');style.id='order-packing-shortcuts-style';style.textContent='.order-reference-shortcuts{display:flex;gap:6px;max-width:100%;overflow:hidden;margin-top:3px}.order-reference-shortcuts button{display:inline-flex;align-items:center;gap:5px;min-width:0;max-width:145px;overflow:hidden;padding:3px 7px;border:1px solid #d6e1ee;border-radius:999px;background:#fff;color:#61758b;font:750 .58rem/1.1 inherit;letter-spacing:.015em;text-align:left;text-overflow:ellipsis;white-space:nowrap;cursor:pointer}.order-reference-shortcuts button:before{content:"";width:8px;height:8px;flex:0 0 8px;border:1.5px solid currentColor;border-radius:50%}.order-reference-shortcuts button:hover{border-color:#8eb2e7;color:#2867bd;background:#f4f8ff}.order-reference-shortcuts button.active{border-color:#73a7eb;background:#eaf3ff;color:#1e5fb8;box-shadow:0 0 0 2px #2d6cdf14}.order-reference-shortcuts button.active:before{border:3px solid #2d6cdf;background:#fff}.order-icon-action.packing-slip{color:#715bd2!important;border-color:#cfc5f0!important;background:linear-gradient(180deg,#fff,#f3f0ff)!important}.order-icon-action.packing-slip:hover{border-color:#9f8ee4!important;background:#eeeaff!important}';document.head.append(style);
  }
  ...
  function addOrderSearchShortcuts(){
    orderShortcutStyle();document.querySelectorAll('.order-item').forEach(item=>{
      ...
      [[asin,'ASIN'],[itemCode&&itemCode!==sku?itemCode:null,'Item code']].forEach(([value,label])=>{if(!value)return;const button=document.createElement('button');button.type='button';button.textContent=value;button.setAttribute('aria-label',label+' '+value+' — select to show matching orders; select again to clear');button.setAttribute('aria-pressed',String(new URL(window.location.href).searchParams.get('q')===value));button.classList.toggle('active',new URL(window.location.href).searchParams.get('q')===value);button.addEventListener('click',()=>applyOrderQuery(value));shortcuts.append(button);});
