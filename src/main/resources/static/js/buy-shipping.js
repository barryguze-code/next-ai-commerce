(()=>{
  function orderShortcutStyle(){
    ...
    const style=document.createElement('style');style.id='order-packing-shortcuts-style';style.textContent='.order-reference-shortcuts{display:flex;flex-wrap:wrap;gap:5px;max-width:100%;margin-top:6px}.order-reference-group{display:contents}.order-reference-group legend{position:absolute;width:1px;height:1px;overflow:hidden;clip:rect(0 0 0 0);white-space:nowrap}.order-reference-options{display:inline-flex;min-width:0}.order-reference-options button{display:inline-flex;align-items:baseline;gap:5px;min-width:0;max-width:170px;overflow:hidden;margin-left:-1px;padding:5px 8px;border:1px solid #cfdcea;border-radius:0;background:#fff;color:#5e748e;font:750 .63rem/1.1 inherit;letter-spacing:.01em;text-align:left;text-overflow:ellipsis;white-space:nowrap;cursor:pointer}.order-reference-options button:first-child{margin-left:0;border-radius:7px 0 0 7px}.order-reference-options button:last-child{border-radius:0 7px 7px 0}.order-reference-options button:only-child{border-radius:7px}.order-reference-options button:hover{position:relative;z-index:1;border-color:#8eb2e7;color:#2867bd;background:#f4f8ff}.order-reference-options button .reference-kind{flex:0 0 auto;color:#7b8da1;font-size:.51rem;font-weight:850;letter-spacing:.075em;text-transform:uppercase}.order-reference-options button .reference-value{min-width:0;overflow:hidden;color:inherit;text-overflow:ellipsis}.order-reference-options button.active{position:relative;z-index:2;border-color:#2d6cdf;background:#2d6cdf;color:#fff;box-shadow:0 0 0 2px #2d6cdf1f}.order-reference-options button.active .reference-kind{color:#dce9ff}.order-icon-action.packing-slip{color:#715bd2!important;border-color:#cfc5f0!important;background:linear-gradient(180deg,#fff,#f3f0ff)!important}.order-icon-action.packing-slip:hover{border-color:#9f8ee4!important;background:#eeeaff!important}';document.head.append(style);
  }
  ...
  function addOrderSearchShortcuts(){
    orderShortcutStyle();document.querySelectorAll('.order-item').forEach(item=>{
      ...
      [[asin?[asin]:[],'ASIN'],[itemCodes,'Item code']].forEach(([values,label])=>{if(!values.length)return;const group=document.createElement('fieldset'),legend=document.createElement('legend'),options=document.createElement('div');group.className='order-reference-group';options.className='order-reference-options';legend.textContent=label;group.append(legend,options);values.forEach(value=>{const button=document.createElement('button');button.type='button';button.innerHTML=`<span class="reference-kind">${label}</span><span class="reference-value"></span>`;button.querySelector('.reference-value').textContent=value;button.setAttribute('aria-label',label+' '+value+' — select to show matching orders; select again to clear');button.setAttribute('aria-pressed',String(new URL(window.location.href).searchParams.get('q')===value));button.classList.toggle('active',new URL(window.location.href).searchParams.get('q')===value);button.addEventListener('click',()=>applyOrderQuery(value));options.append(button);});shortcuts.append(group);});
