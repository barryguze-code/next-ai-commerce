(()=>{
  function orderShortcutStyle(){
    ...
    const style=document.createElement('style');style.id='order-packing-shortcuts-style';style.textContent='.order-reference-shortcuts{display:grid;gap:4px;max-width:100%;margin-top:4px}.order-reference-group{display:flex;align-items:center;gap:5px;min-width:0;margin:0;padding:0;border:0}.order-reference-group legend{flex:0 0 auto;padding:0;color:#7b8da1;font:800 .49rem/1 inherit;letter-spacing:.065em;text-transform:uppercase}.order-reference-options{display:flex;gap:4px;min-width:0;overflow:hidden}.order-reference-options button{display:inline-flex;align-items:center;gap:5px;min-width:0;max-width:125px;overflow:hidden;padding:3px 7px;border:1px solid #d6e1ee;border-radius:999px;background:#fff;color:#61758b;font:750 .58rem/1.1 inherit;letter-spacing:.015em;text-align:left;text-overflow:ellipsis;white-space:nowrap;cursor:pointer}.order-reference-options button:before{content:"";width:8px;height:8px;flex:0 0 8px;border:1.5px solid currentColor;border-radius:50%}.order-reference-options button:hover{border-color:#8eb2e7;color:#2867bd;background:#f4f8ff}.order-reference-options button.active{border-color:#73a7eb;background:#eaf3ff;color:#1e5fb8;box-shadow:0 0 0 2px #2d6cdf14}.order-reference-options button.active:before{border:3px solid #2d6cdf;background:#fff}.order-icon-action.packing-slip{color:#715bd2!important;border-color:#cfc5f0!important;background:linear-gradient(180deg,#fff,#f3f0ff)!important}.order-icon-action.packing-slip:hover{border-color:#9f8ee4!important;background:#eeeaff!important}';document.head.append(style);
  }
  ...
  function addOrderSearchShortcuts(){
    orderShortcutStyle();document.querySelectorAll('.order-item').forEach(item=>{
      ...
      [[asin?[asin]:[],'ASIN'],[itemCodes,itemCodes.length===1?'Item code':'Item codes']].forEach(([values,label])=>{if(!values.length)return;const group=document.createElement('fieldset'),legend=document.createElement('legend'),options=document.createElement('div');group.className='order-reference-group';options.className='order-reference-options';legend.textContent=label;group.append(legend,options);values.forEach(value=>{const button=document.createElement('button');button.type='button';button.textContent=value;button.setAttribute('aria-label',label+' '+value+' — select to show matching orders; select again to clear');button.setAttribute('aria-pressed',String(new URL(window.location.href).searchParams.get('q')===value));button.classList.toggle('active',new URL(window.location.href).searchParams.get('q')===value);button.addEventListener('click',()=>applyOrderQuery(value));options.append(button);});shortcuts.append(group);});
