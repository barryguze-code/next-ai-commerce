(()=>{
 ...
 function icon(name){const image=document.createElement('img');image.src=base+name+(['05-unlocked','06-locked','01-pending','02-unshipped-truck','03-waiting-for-pickup','04-shipped','collaboration','edit-image','human-ai'].includes(name)?'.svg':'.png');image.alt='';image.className='platform-icon';return image;}
 ...
 function init(){
  document.querySelectorAll('.order-item').forEach(row=>{
   ...
   const status=row.querySelector('.amazon-order-status');if(status&&!status.dataset.designReady){status.dataset.designReady='true';const label=status.textContent.trim(),normalized=label.toLowerCase().replace(/[^a-z]/g,'');let kind='other',file;
    ...
    status.dataset.tone=kind;if(file)status.prepend(statusIcon(file));
   }
   const pickup=row.querySelector('.platform-pickup-label');if(pickup&&!pickup.querySelector('img'))pickup.prepend(statusIcon('waiting-for-pickup-final'));
   ...
   if(actions){
    actions.querySelectorAll('.order-icon-action').forEach(b=>{
     ...
     b.replaceChildren(icon(pack?'print-packing-slip':'shipped'));
   ...
   if(product&&!row.querySelector('.order-mapping-inline')){
    ...
    if(source&&product.querySelector('.order-reference-shortcuts')){
     ...
     trigger.title=source.getAttribute('aria-label');trigger.append(icon(source.classList.contains('is-mapped')?'sku-mapped':'sku-not-mapped'));
  ...
  for(const [selector,name] of [['.order-picture-edit','edit'],['.order-picture-more','human-ai']])document.querySelectorAll(selector+':not([data-library-icon])').forEach(button=>{button.dataset.libraryIcon='true';button.querySelector('svg')?.remove();button.prepend(icon(name));});
 }
