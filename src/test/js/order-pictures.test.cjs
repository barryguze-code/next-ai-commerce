...
test('SKU picture menu survives refreshed rows and respects read-only access',async()=>{
 ...
 try{
  ...
  await page.route('**/*',route=>{if(route.request().method()==='POST'){requests++;assert.equal(route.request().headers()['x-csrf-token'],'test');return route.fulfill({json:{imageUrl:'/picture.png'}});}return route.fulfill({contentType:'text/html',body:'<div id="stream-parent"><div data-order-stream><div data-picture-actions data-item-id="item" data-seller-sku="sku" data-product-title="Test product" data-can-edit="true" data-mapping="MAPPED"><span>□</span></div></div></div><input id="buy-shipping-csrf" data-header="X-CSRF-TOKEN" value="test">'});});
  ...
  await page.waitForFunction(()=>document.querySelector('.order-picture-status')?.textContent.includes('updated'));
  ...
  await page.evaluate(()=>{document.querySelector('[data-order-stream]').innerHTML='<div data-picture-actions data-item-id="readonly" data-can-edit="false" data-product-title="Read only"></div>';});
  await page.getByRole('button',{name:'More product actions',exact:true}).click();
