...
test('receiving tabs, expiration batches, fee navigation and overage confirmation',async()=>{
 ...
 try{
  ...
  await page.route('**/*',route=>{
   ...
   return route.fulfill({contentType:'text/html',body:'<main class="receive-work" data-can-edit="true" data-can-add-location="false"><button data-receive="line">Receive</button><table class="rw-table"></table><p id="rw-feedback"></p></main>'+dialogs});
  });
  ...
  await page.locator('[data-batch-quantity]').fill('2');await page.locator('[data-batch-date]').fill('2027-02-16');
  await page.getByRole('tab',{name:'Damaged',exact:true}).click();assert.equal(await page.locator('[name=damaged]').inputValue(),'6');await page.locator('[name=damaged]').fill('2');
